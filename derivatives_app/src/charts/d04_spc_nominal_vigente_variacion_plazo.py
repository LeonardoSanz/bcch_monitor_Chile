...
CHART_ID = "d04_spc_nominal_vigente_variacion_plazo"
TITLE = "D04 - Variación de montos vigentes SPC nominal por plazo residual"
...
def build_dataframe(client: BCCHClient | None, registry: SeriesRegistry, start_date: str, end_date: str, demo: bool = False) -> pd.DataFrame:
    _, plazo_df, _ = extract_spc_nominal_vigentes(start_date, end_date)
    if plazo_df.empty:
...
def build_figure_plazo(df: pd.DataFrame, base_month=None, compare_month=None, apertura: str = APERTURA_TOTAL):
    ...
    return variation_bar(comp, "plazo", f"BANCOS. VARIACIÓN SPC-CLP VIGENTE POR PLAZO RESIDUAL. {apertura.upper()}", PLAZO_ORDER)


def build_figure_ranking(df: pd.DataFrame, base_month=None, compare_month=None, apertura: str = APERTURA_TOTAL):
    comp = build_comparison_dataframe(df, base_month, compare_month, apertura)
    return variation_ranking(comp, "plazo", f"BANCOS. RANKING DE CONTRIBUCIÓN SPC-CLP VIGENTE. {apertura.upper()}")
