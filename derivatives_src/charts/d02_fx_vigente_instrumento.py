...
import pandas as pd
from derivatives_src.charts.d02_fx_helpers import DER_BM_VIG_INST_02, MM_USD, INSTRUMENT_ORDER, _filter_dates, _base, normalize_instrument, stacked_bar_line
...
CHART_ID = "d02_fx_usdclp_vigente_instrumento"
TITLE = "D02 - Montos vigentes totales en derivados USD/CLP por tipo de instrumento"
...
def build_dataframe(client: BCCHClient | None, registry: SeriesRegistry, start_date: str, end_date: str, demo: bool = False) -> pd.DataFrame:
    raw = fetch_bde_table(DER_BM_VIG_INST_02).long.copy()
    ...
    return _base(out, CHART_ID, "instrumento", "instrumento", "DER_BM_VIG_INST_02")


def build_figure(df: pd.DataFrame):
    if df.empty:
        from derivatives_src.charts.siid_style import empty_siid_figure
        return empty_siid_figure("Sin datos de vigentes por instrumento")
