...
CHART_ID = "d04_spc_clp_vigente_plazo_residual"
TITLE = "D04 - SPC CLP montos vigentes por plazo residual"
...
def build_dataframe(client: BCCHClient | None, registry: SeriesRegistry, start_date: str, end_date: str, demo: bool = False) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    return df.groupby(["date", "dimension_1", "label", "unit"], as_index=False)["value"].sum().rename(columns={"dimension_1": "plazo_residual"})


def build_figure(df: pd.DataFrame):
    if df.empty:
        return empty_figure("Faltan series para SPC por plazo residual")
    fig = px.area(df, x="date", y="value", color="plazo_residual", hover_data=["label", "unit"])
