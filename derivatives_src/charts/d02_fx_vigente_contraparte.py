...
import pandas as pd
from derivatives_src.charts.d02_fx_helpers import DER_BM_PPC_02, MM_USD, SECTOR_ORDER, _filter_dates, _base, sector_from_label, stacked_bar_line
...
CHART_ID = "d02_fx_usdclp_vigente_contraparte"
TITLE = "D02 - Montos vigentes en derivados USD/CLP por sector de contraparte"

EXCLUDE_NORMS = {
    "monto vigente comprador", "monto vigente vendedor", "monto vigente neto", "sectores", "notas"
}
...
def build_dataframe(client: BCCHClient | None, registry: SeriesRegistry, start_date: str, end_date: str, demo: bool = False) -> pd.DataFrame:
    ...
    raw["sector"] = raw["sector"].replace({"Monto vigente total, total, USD-CLP": "Total"})
    # Evitar duplicar si el total viene explícito: sector != Total para barras, Total para línea.
    raw.loc[raw["bde_series_norm"].str.startswith("monto vigente total", na=False), "sector"] = "Total"
...
def build_figure(df: pd.DataFrame):
    if df.empty:
        ...
        return empty_siid_figure("Sin datos vigentes por contraparte")
